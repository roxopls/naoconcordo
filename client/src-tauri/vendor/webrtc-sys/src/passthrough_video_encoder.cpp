...
class PassthroughVideoEncoder final : public VideoEncoder {
 public:
  ...
  EncoderInfo GetEncoderInfo() const override {
    ...
    // naoconcordo: the upstream encoder already runs its own rate control and
    // obeys the target forwarded by ForwardPendingRateControl. Without this
    // flag VideoStreamEncoder keeps its FrameDropper on, which here drops
    // frames that are *already encoded*: the next delta frame references a
    // frame that was never sent and the picture breaks until the next IDR.
